"""Status bar implementations for different window managers."""
...
from mewline.shared.widget_container import BoxWidget, ButtonWidget
from mewline.utils.widget_utils import text_icon, setup_cursor_hover
...
from mewline.widgets.system_resources import ResourcesWidget
...
class StatusBarBase:
    ...
    def __init__(self, **kwargs):
        ...
        self._pacman_lock = Lock()
        
        self.pacman_icon = Label(label="󰮯", style_classes="pacman-icon panel-text-icon")
        self.pacman_label = Label(label="..", style_classes="pacman-label")
        
        # Internal container for icon and label
        self.pacman_inner = Box(spacing=4, orientation="h")
        self.pacman_inner.children = [self.pacman_icon, self.pacman_label]
        
        # Main clickable button
        self.pacman_container = ButtonWidget(name="pacman-updates")
        self.pacman_container.add(self.pacman_inner)
        self.pacman_container.show_all()
        self.pacman_container.hide() # Initial state
        
        # Signals and tooltips
        self.pacman_container.set_tooltip_text("Left-click: Refresh\nRight-click: Update System")
        self.pacman_container.connect("button-press-event", self._on_pacman_click)
        setup_cursor_hover(self.pacman_container)

    def _on_pacman_click(self, widget, event):
        if event.button == 1: # Left Click
            self.pacman_label.set_label("..")
            self.update_pacman()
        elif event.button == 3: # Right Click
            exec_shell_command_async("kitty -e yay")
        return True

    def update_pacman(self) -> bool:
        if self._pacman_lock.locked():
            return False
            
        with self._pacman_lock:
            GLib.Thread.new(None, self._update_pacman_thread)
        return True

    def _update_pacman_thread(self):
        try:
            ...
            GLib.idle_add(self._apply_pacman_update, count)
        except Exception as e:
            logger.error(f"[Pacman] Check failed: {e}")
            GLib.idle_add(self._apply_pacman_update, "!")

    def _apply_pacman_update(self, count: str):
        try:
            if not count or not count.isdigit():
                count = "0"
            
            self.pacman_label.set_label(count)
            self.pacman_container.show_all()
        except Exception:
            pass
    ...
    def create_layout(self) -> CenterBox:
        ...
        return CenterBox(
            ...
            start_children=Box(
                ...
                children=[
                    SystemTray(),
                    ResourcesWidget(style="margin-left: 8px;"),
                    Box(hexpand=True),
...
class WaylandStatusBar(WaylandWindow, StatusBarBase):
    ...
    def __init__(self, monitor: int | None = None, **kwargs):
        ...
        # Start pacman update check
        self.update_pacman()
        GLib.timeout_add_seconds(600, self.update_pacman)
