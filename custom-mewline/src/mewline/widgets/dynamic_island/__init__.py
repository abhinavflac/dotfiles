...
logger.debug('DynamicIsland module imported')
...
class DynamicIsland:
    ...
    def __init__(self, monitor: int | None = None):
        ...
        try:
            ...
            logger.info(f"Created Dynamic Island window on monitor {monitor} with adaptive window system (file={__file__})")

            # For X11: set up window properly
            if WindowManagerContext.is_x11():
                self._setup_x11_window()

            self._create_click_overlay()
            if self._click_overlay is not None:
                self.window.connect(
                    "size-allocate",
                    lambda *_: GLib.idle_add(self._update_click_overlay_input_shape),
                )
        ...
        ######################################
        self.window.show()
        logger.debug('DynamicIsland.__init__ completed')

    def _create_click_overlay(self):
        """Create a transparent screen-wide overlay to catch outside clicks."""
        try:
            logger.debug("DynamicIsland: create_click_overlay start")
            overlay_body = Box(
                name="dynamic-island-click-overlay-body",
                v_expand=True,
                h_expand=True,
                style="background-color: rgba(0,0,0,0.0);",
            )
            overlay_body.add_events(
                Gdk.EventMask.BUTTON_PRESS_MASK
                | Gdk.EventMask.BUTTON_RELEASE_MASK
                | Gdk.EventMask.POINTER_MOTION_MASK
            )
            overlay_body.connect(
                "button-press-event", self._on_click_overlay_button_press
            )
            overlay_body.connect(
                "button-release-event", self._on_click_overlay_button_press
            )

            self._click_overlay = create_adaptive_window(
                wayland_kwargs={
                    "layer": "overlay",
            ...
            self._click_overlay.set_name("dynamic-island-click-overlay")
            self._click_overlay.add_events(
                Gdk.EventMask.BUTTON_PRESS_MASK | Gdk.EventMask.BUTTON_RELEASE_MASK
            )
            self._click_overlay.connect(
                "button-press-event", self._on_click_overlay_button_press
            )
            self._click_overlay.connect(
                "button-release-event", self._on_click_overlay_button_press
            )
            if hasattr(self._click_overlay, "set_keep_above"):
                self._click_overlay.set_keep_above(True)
            if hasattr(self._click_overlay, "set_accept_focus"):
                self._click_overlay.set_accept_focus(False)
            if hasattr(self._click_overlay, "set_skip_taskbar_hint"):
                self._click_overlay.set_skip_taskbar_hint(True)
            if hasattr(self._click_overlay, "set_can_focus"):
                self._click_overlay.set_can_focus(True)
            try:
                screen = self._click_overlay.get_screen()
                visual = screen.get_rgba_visual()
                if visual is not None:
                    self._click_overlay.set_visual(visual)
                    self._click_overlay.set_app_paintable(True)
            except Exception:
                ...
            self._click_overlay.hide()
            logger.debug("DynamicIsland: create_click_overlay created overlay")

            display = Gdk.Display.get_default()
            if display is not None:
                monitor = (
                    display.get_monitor(self.monitor)
                    if self.monitor is not None
                    else display.get_primary_monitor()
                )
                if monitor is not None:
                    geometry = monitor.get_geometry()
                    self._click_overlay.resize(geometry.width, geometry.height)
                    self._click_overlay.move(geometry.x, geometry.y)
        except Exception as e:
            logger.warning(f"Failed to create dynamic island click overlay: {e}")
            self._click_overlay = None

    def _show_click_overlay(self):
        if self._click_overlay is None:
            logger.debug("DynamicIsland: show_click_overlay no overlay")
            return False
        try:
            logger.debug("DynamicIsland: show_click_overlay called")
            self._click_overlay.show_all()
            self._click_overlay.present()
            try:
                overlay_window = self._click_overlay.get_window()
                if overlay_window is not None:
                    logger.debug(
                        f"DynamicIsland: overlay window size={overlay_window.get_width()}x{overlay_window.get_height()} visible={self._click_overlay.get_visible()}"
                    )
            except Exception as e:
                logger.warning(f"DynamicIsland: overlay window debug error: {e}")
            self._update_click_overlay_input_shape()
        except Exception as e:
            logger.warning(f"DynamicIsland: show_click_overlay error: {e}")
        return False

    def _hide_click_overlay(self):
        if self._click_overlay is None:
            return False
        try:
            self._click_overlay.hide()
    ...
    def _on_click_overlay_button_press(self, widget, event):
        try:
            logger.debug(
                "DynamicIsland: overlay click event"
                f" type={getattr(event, 'type', None)}"
                f" button={getattr(event, 'button', None)}"
                f" x={getattr(event, 'x', None)} y={getattr(event, 'y', None)}"
            )
            self.close()
            return True
        except Exception as e:
            logger.warning(f"DynamicIsland: overlay button press error: {e}")
            return False

    def _update_click_overlay_input_shape(self):
        if self._click_overlay is None or self.window is None:
            return False

        try:
            content_window = self.window.get_window()
            if content_window is None:
                logger.debug("DynamicIsland: update_click_overlay_input_shape no content_window")
                return False

            display = Gdk.Display.get_default()
            if display is None:
                logger.debug("DynamicIsland: update_click_overlay_input_shape no display")
                return False
            ...
            if monitor is None:
                logger.debug("DynamicIsland: update_click_overlay_input_shape no monitor")
                return False

            monitor_geom = monitor.get_geometry()
            origin = content_window.get_origin()
            origin_x = None
            origin_y = None
            if origin is None:
                logger.debug("DynamicIsland: update_click_overlay_input_shape origin is None")
                return False
            if hasattr(origin, "x") and hasattr(origin, "y"):
                ...
                logger.debug(
                    f"DynamicIsland: update_click_overlay_input_shape bad origin: {origin} type={type(origin)}"
                )
                return False
            ...
            if overlay_window is None:
                logger.debug(
                    "DynamicIsland: update_click_overlay_input_shape no overlay_window"
                )
                return False

            overlay_origin = overlay_window.get_origin()
            if overlay_origin is None:
                logger.debug(
                    "DynamicIsland: update_click_overlay_input_shape no overlay_origin"
                )
                return False

            overlay_x = overlay_origin.x if hasattr(overlay_origin, "x") else overlay_origin[0]
            overlay_y = overlay_origin.y if hasattr(overlay_origin, "y") else overlay_origin[1]
            overlay_width = overlay_window.get_width()
            overlay_height = overlay_window.get_height()
            content_width = content_window.get_width()
            content_height = content_window.get_height()
            content_local_x = origin_x - overlay_x
            content_local_y = origin_y - overlay_y

            logger.debug(
                "DynamicIsland: update_click_overlay_input_shape "
                f"content_origin={origin_x},{origin_y} "
                f"overlay_origin={overlay_x},{overlay_y} "
                f"content_local={content_local_x},{content_local_y} "
                f"content_size={content_width}x{content_height} "
                f"overlay_size={overlay_width}x{overlay_height} "
                f"monitor={monitor_geom.x},{monitor_geom.y},{monitor_geom.width}x{monitor_geom.height}"
            )

            region = cairo.Region()

            def add_region(x, y, width, height):
                if width > 0 and height > 0:
                    region.union(cairo.RectangleInt(int(x), int(y), int(width), int(height)))

            add_region(0, 0, overlay_width, content_local_y)
            add_region(0, content_local_y, content_local_x, content_height)
            add_region(
                content_local_x + content_width,
                content_local_y,
                overlay_width - (content_local_x + content_width),
                content_height,
            )
            add_region(
                0,
                content_local_y + content_height,
                overlay_width,
                overlay_height - (content_local_y + content_height),
            )

            self._click_overlay.input_shape_combine_region(region)
        except Exception as e:
            logger.warning(f"DynamicIsland: update_click_overlay_input_shape error: {e}")
        return False
